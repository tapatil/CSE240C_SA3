...
#define SD_MAX 8192
#define SD_INIT 4096
#define SD_LOWER_TH 4096
#define SD_UPPER_TH 4096
#define SD_MOMENTUM_STEP 1
#define SD_MOMENTUM_MAX 10
#define SD_MOMENTUM_MIN -SD_MOMENTUM_MAX
int sd_counter; // (10-bit)
int sd_momentum; // (10-bit)
...
void sd_update_replacement_state(uint32_t set, uint32_t type, uint8_t hit) {
    ...
    if (hit) {
        ...
        if (type == WRITEBACK) {
            if(SAMPLED_SET(set)) {
                sd_counter += (-1+sd_momentum);
                sd_momentum -= SD_MOMENTUM_STEP;
            } else if(SHIPpp_SAMPLED_SET(set)) {
                sd_counter += (1+sd_momentum);
                sd_momentum += SD_MOMENTUM_STEP;
            }
        ...
        if(sd_momentum < SD_MOMENTUM_MIN) {
            sd_counter = SD_MOMENTUM_MIN;
        } else if (sd_momentum >= SD_MOMENTUM_MAX) {
            sd_counter = SD_MOMENTUM_MAX-1;
        }
        if(sd_counter < 0) {
            sd_counter = 0;
