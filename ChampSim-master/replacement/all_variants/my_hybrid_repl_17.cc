// sd counter update different for different types of misses
...
uint32_t sd_find_set_owner(uint32_t set);
//===================================================================================================
//===================================================================================================
...
#define maxRRPV 3
//uint32_t line_rrpv[MAX_LLC_SETS][LLC_WAYS];// -> rrpv
...
void SHiPpp_initialize_replacement() {
    ...
    for (int i=0; i<MAX_LLC_SETS; i++) {
        for (int j=0; j<LLC_WAYS; j++) {
            ...
            //rrpv[i][j] = maxRRPV; // ALREADY DONE IN HAWKEYE
...
uint32_t SHiPpp_find_victim (uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t PC, uint64_t paddr, uint32_t type) {
    ...
    while (1) {
        for (int i=0; i<LLC_WAYS; i++)
            if (/*line_rrpv*/rrpv[set][i] == maxRRPV) { // found victim
                return i;
            }
        for (int i=0; i<LLC_WAYS; i++)
            //line_rrpv[set][i]++;
